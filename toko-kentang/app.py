...
def load_data():
    doc_ref = db.collection("appdata").document("toko_kentang_data")
    doc = doc_ref.get()
    if doc.exists:
        data = doc.to_dict()
        # Pastikan struktur lengkap agar program aman
        data.setdefault("jurnal_umum", [])
        ...
        # Buat dokumen default kalau belum ada
        data = {
            "jurnal_umum": [],
            "next_id": 1,
            "users": [],
...
def save_data(data):
    doc_ref = db.collection("appdata").document("toko_kentang_data")
    doc_ref.set(data)
    # Sinkron ke session_state agar gampang akses
...
def add_user(email, password, username, role='customer'):
    ...
    users = data.get("users", [])
    # Cek duplikasi user di data.json
    for user in users:
...
def add_order(email, kg, jenis_kentang, alamat, metode_pembayaran, nomor_rekening, ekspedisi):
    ...
    data["orders"] = orders
    save_data(data)
    add_order_jurnal(order)  # tetap buat jurnal penjualan

def add_feedback(email, rating):
    data = st.session_state.data
    feedbacks = data.get("feedbacks", [])
    feedbacks.append({"email": email, "rating": rating})
